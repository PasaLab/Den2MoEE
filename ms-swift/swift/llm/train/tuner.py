...
class TunerMixin:
    ...
    def prepare_model(cls, args, model, *, template=None, train_dataset=None, task_type=None):
        ...
        print("\n\n\n\n\n\n")
        print(f"args.train_type: {args.train_type}")
        print(f"args.is_adapter: {args.is_adapter}")
        print("\n\n\n\n\n\n")
        ...
        if args.is_adapter:
        ...
        elif args.train_type == 'den2moee_dense_router':
            model.train()
            model.requires_grad_(False)
            activate_parameters(model, [], trainable_parameters_regex=r'\bgate\.weight\b')
            ...
            print(f"Frozen parameters:    {frozen_params:,} "
                  f"({frozen_params / total_params * 100:.4f}%)")
            ...
            print("================================\n")
        elif args.train_type == 'den2moee_sparse':
            model.train()
            model.requires_grad_(False)
            activate_parameters(model, [], trainable_parameters_regex=r'\bmlp\.')
            ...
            try:
                ...
                print(f"[WARN] Parameter counting failed: {e}")
            ...
            print(f"Total parameters:     {total_params:,}")
            print(f"Trainable parameters: {trainable_params:,} "
                f"({(trainable_params / total_params * 100 if total_params > 0 else 0):.4f}%)")
            print(f"Frozen parameters:    {frozen_params:,} "
                f"({(frozen_params / total_params * 100 if total_params > 0 else 0):.4f}%)")
